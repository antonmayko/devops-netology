...
variable "vm_imageid" {
  type    = list(object({ vm_imid=string}))
  default = [ {vm_imid="fd8qgvtec9k2btfqvjah"},{vm_imid="fd8s78up10fbjbe5atn7"}]
}

variable "vm_resources" {
  type    = list(object({ vm_name=string, cpu=number, ram=number, disk=number }))
  default = [ {vm_name="gitlabserver", cpu  = 2, ram  = 4, disk = 10},{vm_name="gitlabrunner", cpu  = 2, ram  = 2, disk = 10}]
}
